        ...
        if (cartId || cartByUser) {
            if (cartId) {
                ...
                if (cart.items.length > 0) {
                    let checkProduct = cart.items.find(
                ...
                if (cartByUser.items.length > 0) {
                    ...
                    let checkProduct = cartByUser.items.find(
                        (x) => x.productId.toString() === productId.toString()
                    );
                    if (!checkProduct) {
        ...
        if (removeProduct == 0) {
            if (cart.items.length > 0) {
                ...
                if (proCheck !== undefined) {
                    ...
                    cart.totalItems -= 1;
                    cart.totalPrice = cart.totalPrice - product.price * proCheck.quantity;
                    const val = await cart.save();
                    ...
                    return res
                        .status(200)
                        .json({ status: true, message: "Cart updated", data: updatedCart });
                } else {
                    return res
                        .status(400)
                        .json({ status: false, message: "Cart Product does not exist" });
                }
            }
            if (cart.items.length == 0) {
                return res
            ...
            if (cart.items.length > 0) {
                ...
                if (proCheck !== undefined) {
                    if (proCheck.quantity > 1) {
                        proCheck.quantity -= 1;
                        cart.totalPrice -= product.price;
                        const val = await cart.save();
                        return res
                            .status(200)
                            .json({ status: true, message: "Cart updated", data: val });
                    } else if ((proCheck.quantity = 1)) {
                        cart.items = cart.items.filter(
                            (item) => item.productId.toString() !== productId.toString()
                        );
                        cart.totalItems -= 1;
                        cart.totalPrice =
                            cart.totalPrice - product.price * proCheck.quantity;
                        const val = await cart.save();
                        const updatedCart = await cartModel.findById(cartId).lean();
